...
m = pi/c
```
To do this, we recognize that $\frac{g(Y|x)}{Mg(Y)} = \frac{\pi}{CM}\exp{-\frac{1}{2}(x-y)^2}$, with $CM = C\sup\frac{g(\theta|x)}{g(\theta)} = C sup \frac{\pi}{C}\exp{-\frac{1}{2}(x-\theta)^2}$, which is $\pi$ and occurs when $\theta = x$.  So clearly $M = \frac{\pi}{C}$, which we can calculate to be `r c`.  Thus $M =$ `r m` given our predetermined parameters, and it is important to note that under these conditions, that the probability of acceptance is $\frac{1}{M} = \frac{C}{\pi} = 0.1088$
...
```
The sample sizes is a nice touch, and I should figure out how to do it. I'm still not sure why mine isn't working.
...
This has a very small difference `r abs.diff`.  Our sample size estimate in fact is probably too conservative because this is a very smal difference.  I will test to see if on average this is too conservative
...
for(i in 1:N){
  ...
  k = rmultinorm(1,n,vec.p)
  null.data = data.frame(rbind(k[1:2],k[3:4]))
  multi.lam = likelihood.test(null.data)$statistic
}

order.bin = sort(two.binlam)
order.mult = sort(multi.lam)
```
# Problem 3.10
A lot more involved.  Going to have to work on this one a lot
